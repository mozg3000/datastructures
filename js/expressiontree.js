...
module.exports = class ExpressionTree extends BinaryTree{
	...
	buildSubTree(expression){
		...
		if(expression.length > 1){
			...
			if(!maxPriorityIndexes.length){
				// return new this.Node(_generator(this.range));
				// return new this.Node(_generator(this.range));
				return new this.Node(new Object());
			...
			rootNode = !isNaN(expression[0])?
								new this.Node(expression[0]) :
								new this.Node(new Object());
		}
		return rootNode;
	}
